...
export default function DiaryPage() {
  ...
  const runMoodAnalysis = async () => {
    if (!user || entries.length === 0 || !password) {
      alert('Please ensure you have entries and a password set');
      return;
    }

    setAnalyzing(true);
    try {
      console.log('Calling mood API with userId:', user.id, 'entries count:', entries.length);
      ...
      // Reload entries to reflect updated mood
      await loadEntries(user.id);
    } catch (error: any) {
      console.error('Error analyzing mood:', error);
      alert(`Failed to analyze mood: ${error.message}`);
  ...
  return (
    <div className="min-h-screen p-4 md:p-8">
      <div className="max-w-4xl mx-auto">
        ...
        <Card className="mb-8">
          <h2 className="text-lg font-medium mb-4">
            {password ? 'Change encryption password' : 'Enter your password to decrypt entries'}
          </h2>
          <div className="flex gap-2">
            <input
              type="password"
          ...
          {password && (
            <p className="text-xs text-gray-500 mt-2">
              Current password set. New entries will be encrypted with this password.
            </p>
          )}
        </Card>
